...
Contact::Contact(int cat, QString first, QString last, QString add, QString zip,
                 QString cty, QString num, QObject *parent)
    : QObject(parent), m_Category(cat), m_FirstName(first), m_LastName(last),
...
QString Contact::toString() const {
  QString contactStr = QString("Category: %1\nFirst Name: %2\nLast Name: %3\n")
                           .arg(m_Category)
                           .arg(m_FirstName)
                           .arg(m_LastName)
                           .append(QString("Street Address: %4\nZip Code "
                                           "%5\nCity: %6\nPhone Number: %7\n")
                                       .arg(m_StreetAddress)
                                       .arg(m_ZipCode)
                                       .arg(m_City)
