...
OrderForm::OrderForm(QWidget *parent) : QWidget(parent) {
  ...
  connect(m_quantitySpin, SIGNAL(valueChanged(int)), this,
          SLOT(updateTotalPrice()));
  connect(m_unitPriceSpin, SIGNAL(valueChanged(double)), this,
          SLOT(updateTotalPrice()));
...
void OrderForm::cancel() { this->close(); }
...
void OrderForm::updateTotalPrice() {
  m_totalPriceSpin->setValue(m_quantitySpin->value() *
                             m_unitPriceSpin->value());
}
