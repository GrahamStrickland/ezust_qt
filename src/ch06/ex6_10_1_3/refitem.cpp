...
void RefItem::setNumberOfCopies(int newVal) { m_NumberOfCopies = newVal; }
...
RefItem::RefItem(QStringList &plst)
    : m_ItemType(plst.takeFirst()), m_ISBN(plst.takeFirst()),
...
Book::Book(QString type, QString isbn, QString title, QString author,
    ...
    : RefItem(type, isbn, title, numCopies), m_Author(author), m_Publisher(pub),
      m_CopyrightYear(year) {}

Book::Book(QStringList &plst)
    : RefItem(plst), m_Author(plst.takeFirst()), m_Publisher(plst.takeFirst()),
...
TextBook::TextBook(QString type, QString isbn, QString title, QString author,
                   QString pub, int year, TextCategory textcat, int numCopies)
    : Book(type, isbn, title, author, pub, year, numCopies),
      m_Category(textcat) {}
...
TextBook::TextCategory TextBook::getCategory() const { return m_Category; }
...
Dvd::Dvd(QString type, QString isbn, QString title, QString creator,
         QString pub, int year, int numCopies)
    : RefItem(type, isbn, title, numCopies), m_Creator(creator),
      m_Publisher(pub), m_CopyrightYear(year) {}

Dvd::Dvd(QStringList &proplist)
    : RefItem(proplist), m_Creator(proplist.takeFirst()),
...
int Dvd::getCopyrightYear() const { return m_CopyrightYear; }

QString Dvd::getCreator() const { return m_Creator; }

QString Dvd::getPublisher() const { return m_Publisher; }

Film::Film(QString type, QString isbn, QString title, QString creator,
           QString pub, int year, FilmCategory filmcat, int numCopies)
    : Dvd(type, isbn, title, creator, pub, year, numCopies),
      m_Category(filmcat) {}
...
Film::FilmCategory Film::getCategory() const { return m_Category; }
...
DataBase::DataBase(QString type, QString isbn, QString title, QString creator,
                   QString pub, int year, DBCategory dbcat, int numCopies)
    : Dvd(type, isbn, title, creator, pub, year, numCopies), m_Category(dbcat) {
}

DataBase::DataBase(QStringList &proplist)
    : Dvd(proplist),
...
DataBase::DBCategory DataBase::getCategory() const { return m_Category; }
