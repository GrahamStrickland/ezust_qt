...
ContactList::ContactList() : m_length(0) { m_contactList.reserve(m_length); }

ContactList::ContactList(QList<Contact> list) : m_length(list.size()) {
  m_contactList.reserve(m_length);
}

ContactList::ContactList(unsigned int length) : m_length(length) {
  m_contactList.reserve(m_length);
}
...
QStringList ContactList::getPhoneList(int cat) const {
  ...
  foreach (const Contact &contact, m_contactList) {
    if (contact.getCategory() == cat)
      phoneList
          << QString("%1\t%2").arg(contact.getName()).arg(contact.getNumber());
  }
...
QStringList ContactList::getMailingList(int cat) const {
  ...
  foreach (const Contact &contact, m_contactList) {
    if (contact.getCategory() == cat)
      mailingList
          << QString("%1\t%2").arg(contact.getName()).arg(contact.getAddress());
  }

  return mailingList;
}

ContactList::~ContactList() { m_contactList.clear(); }
