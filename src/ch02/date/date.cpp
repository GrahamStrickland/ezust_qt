...
Date::Date() { setToToday(); }

Date::Date(int y, int m, int d) { set(y, m, d); }

void Date::set(int y, int m, int d) {   // Check for valid date.
...
Date Date::addDays(int days) { // Check that days is non-negative.
  assert(days >= 0);
...
bool Date::leapYear(
    int year) { // Check whether year is divisible by 4, or by400.
  if (year % 4 == 0) {
    if (year % 100 == 0) {
      if (year % 400 == 0)
        return true;
...
void Date::YMD(int &y, int &m,
               int &d) { // Set year to base, month to Jan, days to total.
  y = s_BaseYear;
  m = 1;
